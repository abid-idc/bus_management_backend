...
<table style="margin-bottom: 30px">
    <tr>
        ...
        <td style="float: right; text-align: right; width: 80%">
            <h1 style="float: right; text-align: right; color: #747e7f; padding-right: 20px">Historique des controls</h1>
        </td>
    </tr>
</table>
<table style="margin-bottom: 30px">
    <tr>
        <td style="text-align: left;">
            ...
            @endif
            <p class="normal-text"><b>Historique des controls depuis le </b> {{$start_date . ' jusqu\'a le ' . $end_date}}</p>
        </td>
...
<table class="invoice-table" style="margin-bottom: 30px">
    ...
    <tr>
        ...
        <th>Date</th>
        <th>Status du Bus</th>
        <th>Status de la Route</th>
        <th>Status du Chauffeur</th>
        <th>Status du Receveur</th>
    </tr>
    </thead>
    <tbody>
        @for($i=0; $i<count($employee->controls); $i++)
            <tr>
                <td>{{$employee->controls[$i]->bus->plate_number}}</td>
                <td>{{$employee->controls[$i]->line->depart->name . '-' . $employee->controls[$i]->line->arrival->name}}</td>
                <td>{{$employee->controls[$i]->created_at}}</td>
                <td>{{$employee->controls[$i]->bus_status}}</td>
                <td>{{$employee->controls[$i]->road_status}}</td>
                <td>{{$employee->controls[$i]->driver_status}}</td>
                <td>{{$employee->controls[$i]->receiver_status}}</td>
            </tr>
        @endfor
    </tbody>
</table>
