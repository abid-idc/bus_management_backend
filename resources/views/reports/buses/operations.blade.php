...
<table style="margin-bottom: 30px">
    <tr>
        <td style="text-align: left;">
            <p class="normal-text"><b>Matricule: </b> {{$bus->plate_number ?? '-'}}</p>
            <p class="normal-text"><b>Nom: </b> {{$bus->name ?? '-'}}</p>
            <p class="normal-text"><b>Date d'entrée: </b> {{$bus->entering_date ?? '-'}}</p>
            <p class="normal-text"><b>Status: </b> {{$bus->status ?? '-'}}</p>
            <p class="normal-text"><b>Liste des opérations depuis le </b> {{$start_date . ' jusqu\'a le ' . $end_date}}</p>
        </td>
        <td style="float: right; text-align: right;">
            <img src="{{'data:image/png;base64,' . DNS2D::getBarcodePNG($bus->name, 'QRCODE')}}" alt="QR Code" style="width: 55px; height: 55px; margin-bottom: 5px">
            @php
...
<table class="invoice-table" style="margin-bottom: 30px">
    ...
    <tbody>
        @for($i=0; $i<count($bus->operations); $i++)
            <tr>
                <td>{{$bus->operations[$i]->type->name}}</td>
                <td>
                    @for($j=0; $j<count($bus->operations[$i]->employees); $j++)
                        {{"- " . $bus->operations[$i]->employees[$j]->name}}
                    @endfor
                </td>
                <td>{{$bus->operations[$i]->quantity}}</td>
                <td>{{$bus->operations[$i]->unit}}</td>
                <td>{{$bus->operations[$i]->created_at}}</td>
                <td>{{$bus->operations[$i]->observation}}</td>
            </tr>
        @endfor
    </tbody>
</table>
